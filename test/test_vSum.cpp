...
bool test_vSum() {
	...
	double sumF,			// Holds result of vDotF
			timeF,			// Holds cumulative runtime of vDotF
			sumS,			// Holds result of vDotS
	...
	sumF = vSumF(vec);		// Call vNormF on vec
	end_F = clock();			// Store time

	begin_S = clock();									// Store cycle
	sumS = vSumS(vec);							// Call vNormS on vec
	end_S = clock();									// Store cycle
	timeS = double(end_S - begin_S) / CLOCKS_PER_SEC;	// Compute runtime

	if (sumF != sumS) {		// Return false for unequal serial and parallel results
		return false;
	}
	...
	if (rank == 0) {								// If master, display runtimes and results
		std::cout << "vSumF(vec): " << sumF << ", runtime: " << timeF << std::endl;
		std::cout << "vSumS(vec): " << sumS << ", runtime: " << reducedTimeS/size << std::endl;
	}
